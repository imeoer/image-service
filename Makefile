...
docker-nydusify-smoke: docker-static
	docker run --rm -v ${current_dir}:/nydus-rs --workdir /nydus-rs golang:1.14 make -C contrib/nydusify build-smoke
	docker build -t nydusify-smoke misc/nydusify-smoke
	docker run --rm -v $(current_dir):/nydus-rs -v ~/.cargo/target-fusedev:/nydus-rs/target-fusedev --privileged nydusify-smoke
