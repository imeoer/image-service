...
impl NydusCore {
    ...
    pub fn new(bootstrap: &Path, config: Config) -> Result<Self> {
        ...
        // (as an EROFS device or fill target). Without a cache directory only
        // native layers in a local store qualify: their store file is the
        // device and needs no cache.
            ...
            for info in &raw_blob_infos {
                ...
                if !mapped {
                    return Err(Error::InvalidConfig(format!(
                        "storage.dir is required: blob {} is not a native layer in a local store, so a cache file must back its device",
