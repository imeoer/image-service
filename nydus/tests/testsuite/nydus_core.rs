...
/// Build one `erofs-none` layer of `files` into the store, named by its full
/// blob digest as `nydus build --blob-dir` would.
...
/// A merged image of local `erofs-none` layers runs diskless: the store files
/// are the devices, so the flat view maps them directly and no cache file is
/// created or needed.
#[test]
fn core_maps_local_native_layers_without_a_cache() {
    ...
    let diskless = Config::from_yaml(&format!(
        "backend:\n  type: local\n  config:\n    dir: {}\nprefetch:\n  scope: none\n",
        store.display(),
    ))
    .unwrap();
    let core = NydusCore::new(&bootstrap, diskless).unwrap();
    let blobs = core.blobs.prepare_all().unwrap();
    assert_eq!(blobs.len(), 2);
    for (blob, source) in blobs.iter().zip([&lower, &upper]) {
        assert_eq!(&blob.cache_path, source, "the store file is the device");
        ...
        // The extent reads the layer data region of the store file itself.
    ...
    let data = core.fs.open("/a").unwrap().read().unwrap();
    assert_eq!(data, vec![0xa1u8; 9000]);

    // Chunk-based layers still need a cache directory to back their devices.
    let (chunked, _config, _blob_id, _corpus) = build_flattened_test_image(dir.path());
    let err = NydusCore::new(
        &chunked,
    ...
    .err()
    .unwrap();
    assert!(err.to_string().contains("storage.dir is required"), "{err}");
}
